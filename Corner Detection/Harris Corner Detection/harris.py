...
class Util:
    ...
    """
        ...
        Functions:
            ...
            imageWrite() --> Stores the reult in the system
            imageDisplay() --> Displays the output
...
class HarrisCornerDetection:
    ...
    """
        Responsibility :  To find the corner points in an image using Harris Corner Detection Algorithm 
        Functions :
    ...
    def harrisResponseScoreMatrix(self, gray_image):
        """
            ...
            @return : harris Corner Score Matrix
        """
    ...
    def nonMaximumSuppression(self, M, m, n):
        """
            @args: M --> Corner Score response matrix
            @args: m --> No of rows
            @args: n --> Number of columns
            @return --> M (after removing false positives)
    ...
    def visualizeCornerPoints(self, M, bgr_image, t=1):
        """
            @args: M --> Corner score response Matrix
            @args: bgr_image --> RGB 3Channel Image
            @return : bgr_image --> Image marked with all the computed corner points
        """
        if t == 1:
            kps = np.argwhere(M >0)
        else:
